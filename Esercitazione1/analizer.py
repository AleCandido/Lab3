...
def Xfunction(a,b): #formula per il calcolo della X dalle colonne del file in input
	return a
def Yfunction(a,b): #idem per la Y
	return b
	
def Xerror(a,b,a_err,b_err): #propagazione errore sulle X
	return a_err
...
def fit(directory, file, a_unit, b_unit, f, p0, titolo="", Xlab="", Ylab="", Xfun=Xfunction, Yfun=Yfunction, Xerr=Xerror, Yerr=Yerror, preplot=False, Xscale="linear",Yscale="linear",scarti=False,table=False,Xtab=Xlab, Ytab=Ylab, caption=""):
	...
	#X1,Y1,dX1,dY1 = loadtxt("C:\\Users\\Roberto\\Desktop\\1polar.txt", unpack = True)
	X1,Y1 = loadtxt(directory+"data\\"+file+".txt", unpack = True)
	X=Xfun(X1,Y1)
	Y=Yfun(X1,Y1)
	#dX=1
	#dY=1
	dX1=mme(X,a_unit)
	dY1=mme(Y,b_unit)
	dX=Xerr(X1,Y1,dX1,dY1)
	dY=Yerr(X1,Y1,dX1,dY1)
	...
	#p0=[1,1,1,1] #parametri iniziali
	
	#err=sqrt(dY**2+(df(X,*par)*dX)**2)
	err = dY
	...
	if scarti==True:
		...
		if Xscale=="log":
			xscale("log")
		plot(X, (Y-f(X,*par))/err, ".", color="blue")
